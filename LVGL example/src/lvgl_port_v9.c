 ...
 */
/*******************************************************************************
Modified by profi-max (Oleg Linnik) 2026
...
DRAM_ATTR static TaskHandle_t lvgl_task_handle = NULL;
...
esp_err_t lvgl_port_init(esp_lcd_panel_handle_t lcd_handle, esp_lcd_touch_handle_t tp_handle, lvgl_port_interface_t interface)
{
    ...
    lvgl_mux = xSemaphoreCreateRecursiveMutexStatic(&xMutexBuffer);
    assert(lvgl_mux);
 ...
 //       ESP_LOGI(TAG, "Waiting LVGL task");
    }

#if LVGL_PORT_AVOID_TEAR_ENABLE

    if (interface == LVGL_PORT_INTERFACE_MIPI_DSI_NO_DMA || interface == LVGL_PORT_INTERFACE_MIPI_DSI_DMA) 
    {
        esp_lcd_dpi_panel_event_callbacks_t cbs = {
            .on_refresh_done = mipi_dsi_lcd_on_vsync_event,
        };
        esp_lcd_dpi_panel_register_event_callbacks(lcd_handle, &cbs, NULL);
    }
#endif
